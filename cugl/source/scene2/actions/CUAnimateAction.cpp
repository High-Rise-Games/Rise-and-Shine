 ...
 */
void Animate::start(const std::shared_ptr<SceneNode>& target, void** state) {
 ...
 */
void Animate::stop(const std::shared_ptr<SceneNode>& target, void** state) {
    free(*state);
    *state = NULL;
    SpriteNode* strip = dynamic_cast<SpriteNode*>(target.get());
    int frame = getFrame(1.0f);
 ...
 */
void Animate::update(const std::shared_ptr<SceneNode>& target, void* state, float dt) {
    SpriteNode* strip = dynamic_cast<SpriteNode*>(target.get());
    CUAssertLog(strip,"Attempt to animate a node other than an AnimationNode");
