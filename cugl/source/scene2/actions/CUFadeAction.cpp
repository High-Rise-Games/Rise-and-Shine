 ...
 */
void FadeOut::start(const std::shared_ptr<SceneNode>& target, void** state) {
 ...
 */
void FadeOut::stop(const std::shared_ptr<SceneNode>& target, void** state) {
    Color4f color = target->getColor();
    color.a = 0.0f;
 ...
 */
void FadeOut::update(const std::shared_ptr<scene2::SceneNode>& target, void* state, float dt) {
    Color4f color = target->getColor();
 ...
 * @param state     The address to store the node state
 */
void FadeIn::start(const std::shared_ptr<SceneNode>& target, void** state) {
    Color4f color = target->getColor();
    // Because of clamping, we need to carry along two values this time.
    float* data = (float*)malloc(sizeof(float)*2);
    data[0] = color.a;
 ...
 * @param state     The address to deallocate the node state
 */
void FadeIn::stop(const std::shared_ptr<SceneNode>& target, void** state) {
    Color4f color = target->getColor();
    float* data = (float*)(*state);
    color.a = data[0];
    target->setColor(color);
 ...
 * @param dt        The elapsed time to animate.
 */
void FadeIn::update(const std::shared_ptr<scene2::SceneNode>& target, void* state, float dt) {
    Color4f color = target->getColor();
    float* data = (float*)state;
    float orig = data[0];
    
    data[1] += orig*dt;
    color.a = (data[1] < 0 ? 0.0f : data[1] > 1.0 ? 1.0 : data[1]);
    target->setColor(color);
}
