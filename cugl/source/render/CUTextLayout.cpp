 ...
 */
Mesh<SpriteVertex2> TextLayout::getGlyphBoxes() const {
 ...
 * @return a (line) mesh of the quad outlines for the text glyphs
 */
Mesh<SpriteVertex2> TextLayout::getGlyphBoxes(Rect rect) const {
    Mesh<SpriteVertex2> result;
    result.command = GL_LINES;
 ...
 */
size_t TextLayout::getGlyphBoxes(Mesh<SpriteVertex2>& mesh) const {
    Rect bounds = getBounds();
    size_t total = 0;
    for(auto it = _rows.begin(); it != _rows.end(); ++it) {
        float track = 0;
 ...
 * @return the number of quads generated
 */
size_t TextLayout::getGlyphBoxes(Mesh<SpriteVertex2>& mesh, Rect rect) const {
    Rect bounds = getBounds();
    bounds.intersect(rect);
    size_t total = 0;
    for(auto it = _rows.begin(); it != _rows.end(); ++it) {
