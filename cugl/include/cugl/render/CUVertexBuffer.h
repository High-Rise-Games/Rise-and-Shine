...
private:
    ...
    /** The size of the vertex buffer. This is necessary for Vulkan */
    size_t _size;
...
#pragma mark Constructors
     ...
     * @param size     The size of the vertex buffer in bytes (0 for OpenGL)
     *
     * @return true if initialization was successful.
     */
    bool init(GLsizei stride, size_t size = 0);
     ...
     */
    static std::shared_ptr<VertexBuffer> alloc(GLsizei stride, size_t size = 0) {
        std::shared_ptr<VertexBuffer> result = std::make_shared<VertexBuffer>();
        return (result->init(stride, size) ? result : nullptr);
    }
