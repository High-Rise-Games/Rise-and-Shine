        ...
        if (_currentDirtAmount > 0) {
            ...
            if (!_dirtSelected) {
                ...
                if (dirtCon.didRelease()) {
                    ...
                    _playerVec[0]->setAnimationState(Player::THROWING);
                    _projectileVec[1]->spawnProjectile(playerPos, velocity, snapped_dest, ProjectileSet::Projectile::ProjectileType::DIRT, _currentDirtAmount);
                    _allDirtAmounts[0] = 0;
                    _currentDirtAmount = _allDirtAmounts[0];
...
void TutorialController::draw(const std::shared_ptr<cugl::SpriteBatch>& batch) {    
    ...
    if (_allCurBoards[_id-1] == 0) {
        _windowVec[_id-1]->draw(batch, getSize(), Color4(0,0,0,0));
        ...
        _windowVec[leftId - 1]->draw(batch, getSize(), Color4(0,0,0,0));
        ...
        _windowVec[rightId - 1]->draw(batch, getSize(), Color4(0,0,0,0));
